...
class HybridRadiomicsClassifier:
    ...
    def extract_hybrid_features(self, image: np.ndarray, transform) -> Dict[str, Any]:
        ...
        if self.radiomics_extractor is not None:
            # Convert masks to proper format for PyRadiomics (0 for background, 1 for foreground)
            tumor_mask = self._convert_to_binary_mask(masks.get('tumor', np.zeros_like(image[:,:,0])))
            stroma_mask = self._convert_to_binary_mask(masks.get('stroma', np.zeros_like(image[:,:,0])))
            lymphocyte_mask = self._convert_to_binary_mask(masks.get('lymphocytes', np.zeros_like(image[:,:,0])))
    ...
    def _convert_to_binary_mask(self, mask: np.ndarray) -> np.ndarray:
        """Convert any mask format to PyRadiomics-compatible binary mask (0/1 integers)"""
        if mask is None:
        ...
        if mask.dtype == bool:
            # Boolean mask: True -> 1, False -> 0
            binary_mask = mask.astype(np.uint8)
        elif mask.dtype in [np.float32, np.float64]:
            # Float mask: threshold at 0.5
            binary_mask = (mask > 0.5).astype(np.uint8)
        else:
            # Integer mask: make sure it's binary (non-zero -> 1, zero -> 0)
            binary_mask = (mask > 0).astype(np.uint8)
        
        # Ensure we have a reasonable region size (at least 100 pixels for tumor/stroma, 50 for lymphocytes)
        if binary_mask.sum() < 50:
            # Create a small central region if mask is too small or empty
            h, w = binary_mask.shape
            center_h, center_w = h // 2, w // 2
            region_size = 25  # 25x25 pixel region
            binary_mask[center_h-region_size//2:center_h+region_size//2, 
                       center_w-region_size//2:center_w+region_size//2] = 1
...
Canonical: {predictions['probabilities_by_subtype']['Canonical']:.3f}
Immune: {predictions['probabilities_by_subtype']['Immune']:.3f}
Stromal: {predictions['probabilities_by_subtype']['Stromal']:.3f}
    ...
    if subtype_idx == 0:  # Canonical
        report += """
    CLINICAL IMPLICATIONS (Canonical):
    - Intermediate prognosis (37% 10-year survival)
    - E2F/MYC activation, TERT overexpression
    - NOTCH1 and PIK3C2B mutations common
    - Consider DNA damage response inhibitors
    - With low clinical risk score: >90% survival possible
        """
    elif subtype_idx == 1:  # Immune
        report += """
    ...
    - With low clinical risk score: 94% survival
        ...
        report += """
    CLINICAL IMPLICATIONS (Stromal):
    - Poor prognosis (20% 10-year survival)
    - EMT, angiogenesis, TGF-β activation
    - SMAD3 mutation, high VEGFA amplification
    - Consider anti-angiogenic therapy with bevacizumab
    - Target stromal remodeling pathways
