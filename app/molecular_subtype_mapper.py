...
class SpatialPatternAnalyzer:
    ...
    def detect_immune_highways(self, lymphocyte_mask: np.ndarray) -> dict:
        """Detect linear tracks of lymphocytes penetrating tumor (Immune subtype feature)
        
        Key insight from Pitroda et al.: Immune subtype shows dense band-like peritumoral
        infiltration of CD3+ and CD8+ lymphocytes extending intratumorally.
        """
        if SKIMAGE_AVAILABLE:
            ...
            if SCIPY_AVAILABLE:
                ...
                for i in range(1, num_features + 1):
                    ...
                    # Enhanced criteria based on Pitroda's findings
                    if props.eccentricity > 0.85 and props.major_axis_length > self.min_highway_length:
                        highways.append({
                            'length': props.major_axis_length,
                            'orientation': props.orientation,
                            'eccentricity': props.eccentricity,
                            'penetration_depth': self._calculate_penetration_depth(component, lymphocyte_mask)
                        })
                ...
                # Calculate band-like pattern score
                band_pattern_score = self._calculate_band_pattern_score(lymphocyte_mask)
                
                # Calculate intratumoral extension
                intratumoral_extension = np.mean([h['penetration_depth'] for h in highways]) if highways else 0
            else:
                ...
                band_pattern_score = 0.5  # Default moderate score
                intratumoral_extension = 0.3
        else:
        ...
        return {
            ...
            'band_pattern_score': band_pattern_score,
            'intratumoral_extension': intratumoral_extension,
            'dense_peritumoral_infiltration': band_pattern_score > 0.7
        }
    
    def _calculate_penetration_depth(self, component: np.ndarray, full_mask: np.ndarray) -> float:
        """Calculate how deeply immune cells penetrate into tissue"""
        if SCIPY_AVAILABLE:
            # Distance from edge
            edge_distance = distance_transform_edt(full_mask)
            component_distances = edge_distance[component]
            return np.mean(component_distances) / full_mask.shape[0] if len(component_distances) > 0 else 0
        return 0.3  # Default moderate penetration
    
    def _calculate_band_pattern_score(self, lymphocyte_mask: np.ndarray) -> float:
        """Calculate score for band-like lymphocyte pattern (Immune subtype characteristic)"""
        # Find contours and analyze their shape
        ...
        # Fit an ellipse to check for band-like shape
        if len(largest_contour) >= 5:
            ellipse = cv2.fitEllipse(largest_contour)
            _, (width, height), _ = ellipse
            
            # Band-like patterns have high aspect ratio
            aspect_ratio = max(width, height) / (min(width, height) + 1e-6)
            
            # Calculate solidity (convex hull vs actual area)
            hull = cv2.convexHull(largest_contour)
            hull_area = cv2.contourArea(hull)
            contour_area = cv2.contourArea(largest_contour)
            solidity = contour_area / (hull_area + 1e-6)
            
            # Band pattern score combines aspect ratio and solidity
            band_score = min(1.0, (aspect_ratio / 4.0) * solidity)
            return band_score
        
        return 0.0
    
    def analyze_stromal_barriers(self, stroma_mask: np.ndarray, lymphocyte_mask: np.ndarray) -> dict:
        """Detect stromal barriers preventing lymphocyte penetration (Stromal subtype feature)
        
        Key insight from Pitroda et al.: Stromal subtype shows marked intratumoral and 
        peritumoral fibrosis with peritumorally restricted lymphocytic infiltrate.
        """
        ...
        if SCIPY_AVAILABLE:
            ...
            encased_regions = 0
            fibrotic_capsule_score = 0
            
            for i in range(1, num_regions + 1):
                region = (labeled_regions == i)
                region_edges = cv2.Canny(region.astype(np.uint8) * 255, 50, 150)
                if region_edges.sum() > 0:
                    edge_stroma_overlap = (region_edges & stroma_mask).sum() / region_edges.sum()
                    if edge_stroma_overlap > 0.8:
                        encased_regions += 1
                        fibrotic_capsule_score += edge_stroma_overlap
            
            # Calculate fibrosis density
            fibrosis_density = self._calculate_fibrosis_density(stroma_mask)
            
            # Calculate immune exclusion pattern
            immune_exclusion_score = self._calculate_immune_exclusion_score(stroma_mask, lymphocyte_mask)
        else:
            # Simplified version
            ...
            fibrotic_capsule_score = 0.7 if encased_regions > 0 else 0.3
            fibrosis_density = stroma_mask.sum() / (stroma_mask.shape[0] * stroma_mask.shape[1])
            immune_exclusion_score = exclusion_ratio
        ...
        return {
            ...
            'fibrotic_capsule_score': fibrotic_capsule_score / (encased_regions + 1) if encased_regions > 0 else 0,
            'fibrosis_density': fibrosis_density,
            'immune_exclusion_score': immune_exclusion_score,
            'marked_fibrosis': fibrosis_density > 0.4
        }
    
    def _calculate_fibrosis_density(self, stroma_mask: np.ndarray) -> float:
        """Calculate the density of fibrotic tissue"""
        # Use texture analysis to identify dense fibrotic patterns
        if len(stroma_mask.shape) == 2:
            # Convert to uint8 for texture analysis
            stroma_uint8 = (stroma_mask * 255).astype(np.uint8)
            
            # Calculate texture features
            laplacian = cv2.Laplacian(stroma_uint8, cv2.CV_64F)
            texture_score = np.std(laplacian)
            
            # Dense fibrosis has high texture score and high coverage
            density = (stroma_mask.sum() / stroma_mask.size) * (1 + texture_score / 100)
            return min(1.0, density)
        return 0.5
    
    def _calculate_immune_exclusion_score(self, stroma_mask: np.ndarray, lymphocyte_mask: np.ndarray) -> float:
        """Calculate how effectively stroma excludes immune cells"""
        if SCIPY_AVAILABLE:
            # Create zones around stroma
            stroma_dilated = cv2.dilate(stroma_mask.astype(np.uint8), np.ones((10,10), np.uint8), iterations=2)
            exclusion_zone = stroma_dilated & ~stroma_mask
            
            # Calculate lymphocyte density in exclusion zone vs overall
            exclusion_lymphocytes = (lymphocyte_mask & exclusion_zone).sum()
            exclusion_area = exclusion_zone.sum()
            overall_lymphocyte_density = lymphocyte_mask.sum() / lymphocyte_mask.size
            
            if exclusion_area > 0:
                exclusion_density = exclusion_lymphocytes / exclusion_area
                exclusion_score = 1.0 - (exclusion_density / (overall_lymphocyte_density + 1e-6))
                return max(0, min(1.0, exclusion_score))
        return 0.5
    
    def measure_tumor_interface_sharpness(self, tumor_mask: np.ndarray, stroma_mask: np.ndarray) -> dict:
        """Measure sharpness of tumor-stroma interfaces (Canonical subtype feature)
        
        Key insight from Pitroda et al.: Canonical subtype shows prominent nests of tumor cells
        with minimal CD3+/CD8+ infiltration and sharp, pushing borders.
        """
        # Find tumor edges
    ...
    def detect_lymphoid_aggregates(self, lymphocyte_mask: np.ndarray) -> dict:
        """Detect tertiary lymphoid structures (Immune subtype feature)
        
        Key insight from Pitroda et al.: Immune subtype shows significant overexpression of 
        innate and adaptive immune genes with organized lymphoid structures.
        """
...
class MolecularSubtypeMapper:
    ...
    Canonical: Low immune/stromal, high tumor content, E2F/MYC signaling
    Immune: High lymphocyte infiltration, low fibrosis, favorable prognosis  
    Stromal: High stromal content, fibrosis, EMT signatures
    """
    ...
    def __init__(self, tissue_model):
        ...
        self.subtype_profiles = {
            'canonical': {
                ...
                # Molecular markers from Pitroda et al.
                'molecular_markers': {
                    'E2F_MYC_activation': True,
                    'TERT_overexpression': True,
                    'NOTCH1_mutation': True,
                    'PIK3C2B_mutation': True,
                    'VEGFA_amplification': 'moderate',
                    'DNA_damage_signaling': 'abnormal',
                    'cell_cycle_checkpoints': 'disrupted'
                },
                'histological_features': {
                    'tumor_nests': 'prominent',
                    'CD3_CD8_infiltration': 'minimal',
                    'fibrosis': 'minimal',
                    'interface_pattern': 'sharp_pushing_borders'
                },
                'prognosis': {
                    '10_year_survival': 0.37,
                    'with_low_CRS': 0.90,  # Clinical Risk Score 0-1
            ...
            'immune': {
                ...
                # Molecular markers from Pitroda et al.
                'molecular_markers': {
                    'MSI_independent_immune': True,
                    'interferon_alpha_gamma': 'high',
                    'p53_pathway': 'activated',
                    'NRAS_mutation': True,
                    'CDK12_mutation': True,
                    'EBF1_mutation': True,
                    'VEGFA_amplification': 'low',
                    'adaptive_immune_genes': 'overexpressed'
                },
                'histological_features': {
                    'lymphocyte_pattern': 'dense_band_like_peritumoral',
                    'CD3_CD8_infiltration': 'extending_intratumorally',
                    'fibrosis': 'minimal',
                    'tertiary_lymphoid_structures': 'present'
                },
                'prognosis': {
                    '10_year_survival': 0.64,
                    'with_low_CRS': 0.94,
                    'oligometastatic_potential': 'high',  # Limited disease on recurrence
                    'recurrence_pattern': 'limited_1_3_metastases'
                }
            },
            'stromal': {
                'name': 'Stromal',
                ...
                'molecular_markers': {
                    'EMT_signatures': 'high',
                    'angiogenesis': 'high',
                    'extracellular_matrix': 'remodeling',
                    'KRAS_signaling': 'activated',
                    'SMAD3_mutation': True,
                    'VEGFA_amplification': 'high',
                    'TGF_beta_signaling': 'activated',
                    'hypoxia_signatures': 'present'
                },
                'histological_features': {
                    'fibrosis': 'marked_intratumoral_peritumoral',
                    'lymphocyte_pattern': 'peritumorally_restricted',
                    'immune_exclusion': True,
                    'stromal_barriers': 'present',
                    'angiogenic_pattern': 'high'
                },
                'prognosis': {
                    '10_year_survival': 0.20,
                    'with_high_CRS': 0.19,
                    'oligometastatic_potential': 'low',
                    'metastatic_pathways': 'activated'
                }
            }
        }
    ...
    def compute_spatial_enhanced_signatures(self, tissue_probs, spatial_patterns, architecture_score, hist_features):
        ...
        if hist_features.get('solid_nest_score', 0) > 0.5:
            ...
            confidence_reasons.append("Solid tumor nests detected")
                ...
                # Immune features - enhanced based on Pitroda et al.
        ...
        if spatial_patterns['immune_highways'].get('dense_peritumoral_infiltration', False):
            scores[1] += 0.12
            confidence_reasons.append("Dense band-like peritumoral infiltration (key Immune feature)")
            
        if spatial_patterns['immune_highways'].get('intratumoral_extension', 0) > 0.5:
            scores[1] += 0.08
            confidence_reasons.append("CD3+/CD8+ lymphocytes extending intratumorally")
            
        if spatial_patterns['lymphoid_aggregates']['aggregates_present']:
            scores[1] += 0.1
            confidence_reasons.append("Lymphoid aggregates present")
        
        # Stromal features - enhanced based on Pitroda et al.
        ...
        if spatial_patterns['stromal_barriers'].get('marked_fibrosis', False):
            scores[2] += 0.12
            confidence_reasons.append("Marked intratumoral/peritumoral fibrosis (key Stromal feature)")
            
        if spatial_patterns['stromal_barriers'].get('immune_exclusion_score', 0) > 0.7:
            scores[2] += 0.08
            confidence_reasons.append("Immune-excluded microenvironment")
            
        if spatial_patterns['stromal_barriers'].get('fibrotic_capsule_score', 0) > 0.6:
            scores[2] += 0.08
            confidence_reasons.append("Fibrotic encasement pattern")
    ...
    def compute_molecular_signatures(self, tissue_probs, architecture_score, hist_features):
        ...
        # Normalize scores
        scores = np.array([canonical_score, immune_score, stromal_score])
        scores = np.clip(scores, 0, 1)
        
        # Apply biological constraints
        if immune_score > 0.4 and stromal_score < 0.2:
            scores[1] *= 1.3  # Boost Immune
            scores[2] *= 0.7  # Reduce Stromal
        elif stromal_score > 0.4 and immune_score < 0.2:
            scores[2] *= 1.3  # Boost Stromal
            scores[1] *= 0.7  # Reduce Immune
        elif tumor_score > 0.5 and immune_score < 0.15:
            scores[0] *= 1.2  # Boost Canonical
    ...
    def classify_molecular_subtype(self, image, transform, detailed_analysis=True):
        ...
        if self.using_revolutionary and self.revolutionary_predictor:
            ...
            probs = np.array([
                revolutionary_results['probabilities']['Canonical'],
                revolutionary_results['probabilities']['Immune'],
                revolutionary_results['probabilities']['Stromal']
            ])
        ...
        if self.using_advanced and self.advanced_predictor:
            ...
            # Convert probabilities to numpy array
            probs = np.array([
                advanced_results['probabilities']['Canonical'],
                advanced_results['probabilities']['Immune'],
                advanced_results['probabilities']['Stromal']
            ])
        ...
        elif predicted_idx == 0:  # Canonical
            results['risk_category'] = 'Intermediate Risk - Variable Outcomes (37% 10-year survival)'
        else:  # Stromal
            results['risk_category'] = 'High Risk - Aggressive Disease (20% 10-year survival)'
