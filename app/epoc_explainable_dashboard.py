...
class EPOCExplainableDashboard:
    ...
    def _render_ai_assistant(self):
        ...
        user_query = st.text_input(
            ...
            value=query_value,
            placeholder="e.g., 'Explain in simple terms' or 'What does immune mean?'",
...
What would you like to know?"""
    ...
    def _explain_in_simple_terms(self) -> str:
        ...
        if hasattr(st.session_state, 'last_prediction'):
            ...
            simple_explanations = {
                'canonical': """
**Think of canonical like a "standard" cancer** 🎯

Imagine cancer cells as a rebellious group in your body. canonical is like the "textbook" version:
...
""",
                'immune': """
**Think of immune like a "battle zone" cancer** 🛡️
...
""",
                'stromal': """
**Think of stromal like a "fortress" cancer** 🏰
...
- **canonical** = Sunny day (standard, predictable)
- **immune** = Thunderstorm (lots of activity, but nature is fighting back)
- **stromal** = Fog (hard to see through, creates barriers)
...
"""
        ...
        if 'canonical' in subtype:
            explanation += """
• **High tumor content** ({:.1%}) - canonical typically shows dense tumor regions
• **Low immune infiltration** - Minimal lymphocyte presence
• **Canonical pathway activation** - Classic CRC molecular signatures
• **Glandular architecture** - Well-formed tumor glands characteristic of canonical
""".format(tissue_comp.get('Tumor', 0))
            
        elif 'immune' in subtype:
            explanation += """
• **Significant immune infiltration** ({:.1%} lymphocytes) - Key immune marker
...
""".format(tissue_comp.get('Lymphocytes', 0))
        ...
        elif 'stromal' in subtype:
            explanation += """
• **High stromal content** ({:.1%}) - Dominant feature of stromal
...
• canonical (Canonical): {probs[0]:.1%}
• immune (Immune): {probs[1]:.1%}
• stromal (Stromal): {probs[2]:.1%}
...
**Clinical Insight:** Similar immune cases showed better outcomes with combination therapy."""
        else:
    ...
    def _assess_model_readiness(self) -> str:
        ...
        if 'epoc_metrics' in st.session_state:
            ...
            avg_auroc = np.mean([metrics.get(f'{s}_auroc', 0) for s in ['canonical', 'immune', 'stromal']])
...
These features are weighted differently for each subtype, with the AI learning optimal combinations from thousands of training cases."""
    ...
    def _simulate_epoc_predictions(self, epoc_df):
        ...
        for _, row in epoc_df.iterrows():
            ...
            if np.random.random() < 0.85:
                ...
                # Misclassification
                other_subtypes = [s for s in ['canonical', 'immune', 'stromal'] if s != true_subtype]
                pred_subtype = np.random.choice(other_subtypes)
                confidence = np.random.uniform(0.4, 0.7)
            
            # Generate probability distribution
            probs = np.random.dirichlet([1, 1, 1])
            if pred_subtype == 'canonical':
                probs[0] = confidence
            elif pred_subtype == 'immune':
                probs[1] = confidence
            else:
                probs[2] = confidence
    ...
    def _render_confusion_analysis(self):
        ...
        **🎯 What is a Confusion Matrix?** This grid shows where the AI gets predictions right (diagonal) 
        ...
        st.markdown("""
        - **canonical ↔ stromal**: Sometimes confused due to overlapping stromal patterns
        - **immune**: Most distinctive due to immune infiltration
    ...
    def _render_subtype_metrics(self):
        ...
        **🧬 Subtype-Specific Performance** - How well the AI identifies each molecular subtype:
        ...
        subtypes_info = {
            'canonical': {'color': '#ff006e', 'icon': '🧬', 'desc': 'Canonical pathway'},
            'immune': {'color': '#00ff88', 'icon': '🛡️', 'desc': 'Immune-enriched'},
            'stromal': {'color': '#ff9a00', 'icon': '🔥', 'desc': 'Stromal-rich'}
        }
    ...
    def _create_roc_curves(self, results_df):
        ...
        colors = {'canonical': '#ff006e', 'immune': '#00ff88', 'stromal': '#ff9a00'}
    ...
    def _display_clinical_recommendations(self):
        ...
        if hasattr(st.session_state, 'last_prediction'):
            pred = st.session_state.last_prediction
            subtype = pred['subtype'].split()[0]  # Extract canonical/2/3
            
            if subtype == 'canonical':
                st.info("""
                **canonical (Canonical) Recommendations:**
                ...
                **immune (Immune) Recommendations:**
                ...
                st.warning("""
                **stromal (Stromal) Recommendations:**
