...
class UncertaintyAwareEnsemble:
    ...
    def predict_with_uncertainty(self, image, transform=None, return_all_predictions=False):
        ...
        results = {
            ...
            'probabilities': {
                'canonical': float(ensemble_probs[0]),
                'immune': float(ensemble_probs[1]),
                'stromal': float(ensemble_probs[2])
            },
    ...
    def _get_model_prediction(self, name, model, image, transform):
        ...
        if hasattr(model, 'predict'):
            ...
            probs = np.array([
                result.get('probabilities', {}).get('canonical', 0),
                result.get('probabilities', {}).get('immune', 0),
                result.get('probabilities', {}).get('stromal', 0)
            ])
