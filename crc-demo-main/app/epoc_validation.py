...
class EPOCValidator:
    ...
    def process_epoc_cohort(self, epoc_manifest_csv, wsi_directory):
        ...
        Args:
            epoc_manifest_csv: Path to CSV with columns:
                ...
                - molecular_subtype (ground truth: SNF1/SNF2/SNF3)
        ...
        for idx, row in tqdm(epoc_df.iterrows(), total=len(epoc_df)):
            ...
            try:
                ...
                if self.use_hybrid_classifier and 'probabilities_by_subtype' in prediction:
                    ...
                    predicted_subtype = prediction['subtype'].split()[0]  # Extract SNF1/2/3
                    results.append({
                        'patient_id': row['patient_id'],
                        'true_subtype': row['molecular_subtype'],
                        'predicted_subtype': predicted_subtype,
                        'confidence': prediction['confidence'],
                        'snf1_prob': prediction['probabilities_by_subtype'].get('SNF1 (Canonical)', 0),
                        'snf2_prob': prediction['probabilities_by_subtype'].get('SNF2 (Immune)', 0),
                        'snf3_prob': prediction['probabilities_by_subtype'].get('SNF3 (Stromal)', 0),
    ...
    def calculate_validation_metrics(self, results_df):
        ...
        subtype_map = {'SNF1': 0, 'SNF2': 1, 'SNF3': 2}
    ...
    def _calculate_snf2_benefit(self, results_df):
        """Calculate surgical benefit for predicted SNF2 patients"""
        snf2_mask = results_df['predicted_subtype'] == 'SNF2'
        if snf2_mask.sum() > 10:
    ...
    def _calculate_snf3_resistance(self, results_df):
        """Calculate cetuximab resistance in predicted SNF3 patients"""
        snf3_cetux_mask = (
            (results_df['predicted_subtype'] == 'SNF3') & 
